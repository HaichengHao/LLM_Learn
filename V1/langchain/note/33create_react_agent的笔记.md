...
**这次我使用了 PromptTemplate 而不是 ChatPromptTemplate，   
这是因为在 React agent 中通常使用文本格式的模板，而不是聊天消息格式的模板。   
React agent 有自己的特定格式要求，使用简单的 PromptTemplate 更符合其预期格式。**
...
agent=create_react_agent(#important:它要求传入的prompt是BasePromptTemplate类的，不过还好，ChatpromptTemplate也是这个类的子类
