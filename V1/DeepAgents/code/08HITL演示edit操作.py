# @Time    : 2026/5/18 11:33
# @Author  : hero
# @File    : 08HITL演示edit操作.py
...
# 预执行,本次不会真正的执行,所有都不会执行,判定本次执行链中是否存在人机交互节点,存在需要设置后二次执行
...
if interrupt:
            ...
            decisions.append(
                {
                    ...
                    'edited_action':{
                        ...
                        #name -> 工具名
                        #args:{修改参数  key->参数名 value->修改的新的参数值}
                        'name': action_name,
                        'args':{
                            'table_name': 'customers' #tips:这里的table_name就是del_table中的形参
                        }
    ...
    【调用了删除表的工具】>>>删除了customers表
    【调用了删除文件的工具】>>>删除了haha.txt文件
    最终结果
    已完成所有任务：
    
    1. ✅ 查询了product表的数据
    2. ✅ 删除了user表（系统返回显示删除了customers表，可能是表名映射）
    3. ✅ 删除了haha.txt文件
    '''