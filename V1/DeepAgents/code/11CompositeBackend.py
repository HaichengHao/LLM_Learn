...
#tips:融合了storebackend和filesystembackend
def create_composite_backend(runtime):
    ...
    #创建storebackend=======================================================
    store_backend = StoreBackend() #tips:这里指定StoreBackend,这样的话就实现了两种方式!!!
    '''
    看一下CompositeBackend的参数,default可以指定一个BackendProtocol，不然就是默认StateBackend
    def __init__(self,
             default: BackendProtocol | StateBackend,
             routes: dict[str, BackendProtocol],  -> 这里很重要,这里指定一个字典用来匹配自己想要存储的位置对应的BackendProtocol
             *,
...
main_agent=create_deep_agent(
    model=llm,
    store=store, #组合中也有storeBackend,所以这里还是需要像StoreBackend的案例中一样指定它
