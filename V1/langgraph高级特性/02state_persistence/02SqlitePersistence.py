# @Time    : 2026/4/16 09:47
# @Author  : hero
# @File    : 02数据库检查点sqlite.py
...
def main():
    # 数据存储到./目录下面，需要目录存在
    conn = sqlite3.connect(database="./sqlite_data.db", check_same_thread=False) #tips:可以看一下，这样得到的是一个Connection对象
    ...
    #tips:或者是这样写
    # graph_new=builder.compile(checkpointer=SqliteSaver(conn=sqlite3.connect(database='./demo.db',check_same_thread=False)))

    #同一个用户id下，每次执行都会插入一次新数据，测试时记得修改用户编号或者直接删除./sqlite_data.db
