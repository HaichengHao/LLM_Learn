...
def sub_node(state:DemoState) -> DemoState:
...
# 创建子图构建器并配置节点和边
subgraph_builder = StateGraph(DemoState)
...
builder.add_node("subgraph_node", subgraph) #important:将子图作为节点
...
#important:子图调用的状态传递逻辑当主图调用子图节点时，整个过程会触发两次状态合并：也就是主图的状态和子图的返回的带有主图的状态再次拼接
#执行图并打印结果
