...
def create_task_description_handoff_tool(*, agent_name: str, description: str | None = None):
    ...
    def handoff_tool(
        ...
        return Command(
            goto=[Send(agent_name, agent_input)],  #tips:可以传入节点名序列,
            graph=Command.PARENT, #important:用到了之前的父图导航
        )
...
#tips:调用了create_task_description_handoff_tool得到了一个
# Command(
#             goto=[Send("flight_assistant", "将任务移交给航班预订助手")],
#             graph=Command.PARENT, #important:用到了之前的父图导航
#         )
