# @Time    : 2026/4/14 16:29
# @Author  : hero
# @File    : 28api_command基本用法之二.py
"""
CommandDemo.py | LangGraph 1.0.6 正式版
Command 基础演示：状态更新+流程控制+动态路由
"""
'''
学了command之后的疑问，条件边和command好像都能决定下一跳去哪里的问题，那么区分在哪里呢


条件边只会路由到下一个节点而command不仅可以路由到下一个node节点,还支持状态更新,如果需要同时更新状态和路由到不同的节点时,就使用command
'''
...
def decision_agent(state: AgentState) -> Command[AgentState]:
    ...
    # 提取消息文本（兼容空消息）
    last_message = state["messages"][-1] if state["messages"] else ("", "")  #('user', '我需要计算数学题')
    last_msg_content = last_message[1] #('user', '我需要计算数学题')第二个元素,也就是“我需要计算数学题”
    print(f"最新消息文本: {last_msg_content}") #“我需要计算数学题”
