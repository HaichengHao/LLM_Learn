...
## 定义get_session_history  
定义一个方法利用`SQLChatMessageHistory`构造链接数据库并将对话历史存入数据库的功能  
该功能要求传入sid(session_id)  
然后返回一个`BaseChatMessageHistory`的子类`SQLChatMessageHistory`对象
...
## 构建RunnableWithMessageHistory   

构建RunnableWithMessageHistory对象,传入LCEL链,它可以帮助我们实现上下文  
还需要传入`BaseChatMessageHistory`,不过我们上面已经实现了,我们利用的是它的子类
`SQLChatMessageHistory`对象

注意传入的`input_messages_key`是我们上面定义的message的模板字符串
并且传入的`history_messages_key`也是我们设置`MessagesPlaceholder`时设置的变量名,要对应上才行
```python
...
## 调用测试
这次不用chain.invoke而是用我们构造的runnable来invoke
注意,传入config配置并且设置session_id,设置的session_id相同的话
langchain就会"告诉"大模型我们还在聊
```python
